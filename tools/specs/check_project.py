...
def _validate_project_metadata(root: Path, pyproject: dict[str, Any]) -> dict[str, Any]:
    ...
    if match is None or match.group(1) != "0.2.1":
        raise ProjectCheckError("runtime version source must be 0.2.1")
    ...
    if dependencies != ["pyowl-core>=0.2.1,<0.3"]:
        raise ProjectCheckError("pyowl-core>=0.2.1,<0.3 must be the sole runtime dependency")
    return project
