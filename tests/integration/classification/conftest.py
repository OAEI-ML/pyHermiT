...
def _compiled(program: ClauseProgram) -> CompiledOntology:
    fingerprint = _Fingerprint(b"c" * 32)
    named = tuple(
        value.identifier
...
@pytest.fixture
def make_classification() -> Callable[..., ClassificationHarness]:
    def make(
        ...
        *,
        ...
        source = tuple(axioms)
        normalized = normalize_axioms(source, logical_fingerprint="c1" * 32)
        ...
        def temporary(extra: tuple[owl.AxiomNode, ...]) -> CheckResult:
            ...
            full = normalize_axioms(source + extra, logical_fingerprint="c2" * 32)
        ...
        entailment = EntailmentService(executor)
        classification = ClassificationService(
            entailment,
